...
class Pixel_Matching:
    ...
    def imageDepthCallback(self, data):
        ...
        try:
            ...
            # Split the depth image into chunks for parallel processing
            chunk_size = (self.ROI[1] - self.ROI[0]) // cpu_count()
            x_ranges = [(i, min(i + chunk_size, self.ROI[1])) for i in range(self.ROI[0], self.ROI[1], chunk_size)]

            # Create a list of arguments for parallel processing
            depth_intrinsic_params = (
                ...
                self.depth_intrinsics.model, self.depth_intrinsics.coeffs
            )
            rgb_intrinsic_params = (
                self.RGB_intrinsics.width, self.RGB_intrinsics.height,
                self.RGB_intrinsics.ppx, self.RGB_intrinsics.ppy,
                self.RGB_intrinsics.fx, self.RGB_intrinsics.fy,
                self.RGB_intrinsics.model, self.RGB_intrinsics.coeffs
            )
            depth_2_RGB_extrinsics = (self.depth_to_RGB.rotation, self.depth_to_RGB.translation)

            args_list = [(cv_depth_image, depth_intrinsic_params, depth_2_RGB_extrinsics, rgb_intrinsic_params, self.ROI, x_range) for x_range in x_ranges]

            # Perform parallel projection using multiprocessing
            results = self.pool.starmap(project_from_depth_to_RGB_chunk, args_list)

            # Combine the results from parallel processing
            self.projected_depth_on_RGB = np.zeros((self.RGB_intrinsics.height, self.RGB_intrinsics.width), dtype=np.uint16)
            for result in results:
                self.projected_depth_on_RGB += result
...
def project_from_depth_to_RGB_chunk(depth_image, depth_intrinsic_params, depth_2_RGB_extrinsics, rgb_intrinsic_params, ROI, x_range):
    depth_intrinsic = rs2.intrinsics()
    depth_intrinsic.width, depth_intrinsic.height, depth_intrinsic.ppx, depth_intrinsic.ppy, depth_intrinsic.fx, depth_intrinsic.fy, depth_intrinsic.model, depth_intrinsic.coeffs = depth_intrinsic_params

    rgb_intrinsic = rs2.intrinsics()
    rgb_intrinsic.width, rgb_intrinsic.height, rgb_intrinsic.ppx, rgb_intrinsic.ppy, rgb_intrinsic.fx, rgb_intrinsic.fy, rgb_intrinsic.model, rgb_intrinsic.coeffs = rgb_intrinsic_params

    depth_2_RGB = rs2.extrinsics()
    depth_2_RGB.rotation, depth_2_RGB.translation = depth_2_RGB_extrinsics
